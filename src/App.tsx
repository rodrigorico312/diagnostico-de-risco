  ...
  const handleCheckout = async () => {
    ...
    try {
      const response = await axios.post('/api/create-checkout', {
        productId: 'diagnostico-patrimonial',
        productName: 'Diagnóstico de Risco Patrimonial',
        ...
        <FadeIn className="max-w-[560px] mx-auto">
          ...
          <div className="rounded-lg p-6 text-center" style={{ backgroundColor: card, border: `1px solid ${gold}20` }}>
            ...
            <button 
              ...
              disabled={loading}
              className="inline-flex items-center justify-center gap-2 w-full font-bold py-3 px-6 rounded-lg text-sm transition-all hover:brightness-90 disabled:opacity-50 disabled:cursor-not-allowed" 
              style={{ backgroundColor: gold, color: bg1 }}
            >
              {loading ? (
