(() => {
  ...
  const navigation = document.getElementById("psychology-navigation");
  ...
  const disclosureQuery = window.matchMedia("(max-width: 580px)");
  const contentDisclosures = Array.from(document.querySelectorAll(".content-disclosure"));
  const syncContentDisclosures = () => {
    contentDisclosures.forEach((item, index) => {
      item.open = !disclosureQuery.matches || index === 0;
    });
  };
  syncContentDisclosures();
  disclosureQuery.addEventListener("change", syncContentDisclosures);

  window.addEventListener("hashchange", () => {
    const target = document.querySelector(window.location.hash);
    if (target?.matches(".content-disclosure")) target.open = true;
  });
